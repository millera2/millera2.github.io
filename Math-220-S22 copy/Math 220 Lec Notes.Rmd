...
Before: "two t test"  for two means

H0:  m1 - mu2 = 0    ( ie:  mu1 = mu2)

Paired data:  special version of this test.  Better!  Simpler!  Higher power!


"Paired"  each observation in one sample has natural connection to EXACTLY ONE in the other.

Note:  same n!

Practically:  "before and after"

Pairing:  same people!

Ex:  A researcher wonders:  does listening to Mozart increase IQ?

3 people participate:  

1)  take IQ test
2)  listen to Mozart for a month
3)  take IQ test again
...
If there really is no mean difference in IQ, then there's a 22.2% chance of observing a mean difference less than -1.67.

4)  Conclusion

Since p>.05, we fail to reject H0.    There's not strong evidence of an increase in IQ after Mozart.
...
xkcd moral of the story (jelly beans):  Type I error chance is **additive** over multiple tests.

Q:  what if you've got more than 2 groups?

ANOVA is like a 2-t test for means, for MORE THAN 2 GROUPS!

Avoids the problem of increasing type I error chance with multiple tests!

Idea for test stat:  compare:
...
Huge p-val!  Fail to reject!  Not evendience of a difference!
...
Q:  How do sepal lengths compare for the 3 species?
...
Is there evidence of a difference in mean sepal length?
...
test stat:   F = 119.3, num df = 2, den df = 147
...
Since p<< .05, we reject H0.  We've found strong evidence that at last one mean differs.
...
Q:  how does car class affect city mpg?  evidence of a difference?

```{r}
boxplot(mpg$cty~mpg$class, horizontal = T)
```
...
Strong evidence of a differnce in mean.


```{r}
summary( aov( cty~class, data = mpg))
```


## Coeff of Determination

    SSTr = varation between groups
    SSE  = varation within groups
    ...
    Coeff of det =
                 ...
                 = percentage of variation determined by group 
                   membership
                   
  Ie, how much affect does the group have on the observations?
  
```{r}
aov( Sepal.Length~Species, data = iris) %>% summary()
```

SSTr = 63.21,   SSE = 38.96
...
Of all the variation in sepal length, about 61.9% is because of the species.



## CH 12 - Correlation and Linear Regression

Ex)  mpg.  Q:  Is there a relationship btwn cty mpg and hwy mpg?

SCATTERPLOT!  Visual for two quant variables!
...
Woah!  Looks like higher cty mpg is associated with higher hwy mpg!

Ex)  Is there rel btwn displ (engine size) and cty mpg.
...
Looks like bigger engines are less fuel efficient.

Ex)  iris.  Is there rel btwn sepal lenght and sepal width?
...
Hm.  Doesn't seem to be a relationship.

In 220:  care about LINEAR relationships.

## r - correlation coefficient

The correlation coeff, r, is a measure of both the STRENGTH and the DIRECTION of a linear relationship.

R facts:

1)    -1 <= r <= 1    FOR ANY DATA

2)  r close to +1  ->  stronger + linear relationship
    r close to -1  ->  stronger - lin rel
    r close to 0   ->  little/no LINEAR relationship
    
3)  r is unitless

4)  r doesn't depend on units for x or y.
    Ex:  convert inches -> cm, doesn't affect r!
    (Whew!  Relief!  Don't care what units our data has!)
    
5)  doesn't matter which is x, which is y.
...
WARNING:  for equation of the best fit line,  IT DOES MATTER WHICH IS X, WHICH IS Y!
...
Fortunately:  they're all the same!  ONE UNIQUE BEST LINE!

Q:  what makes it the best?

A:  it minimizes the SQUARE RESIDUALS.

Note:  for all data,  sum (residuals) = 0

Idea:  the line makes predictions ABOUT Y, BASED ON X.
...
here:
   ...
   where b0 = intercept, b1 = slope
   
To compute:

   slope = b1 = r*sy/sx
   
Q:  If correlation is stronger, what effect on the slope?

A:  It gets steeper!  Strong correlation -> x has bigger effect on y!

Fact:  the least square line always touches the point (xbar,ybar)


    int = b0 = ybar - b1*xbar
    
Ex)  Find the equation for the model to predict cty mpg based on displ
...
-->   yhat = -0.363 + 0.416x
     ...
     slope:  If a flower's petal length increases by 1cm, we
             expect/predict its petal width to increase by 
             0.416 cm.
...
1)  slope
    ...
    b1 = (change in y)/(change in x)
       = how much y changes from 1 unit increase in x

In context:  If our x-variable increases by 1 unit, then we predict/expect y-varible will inc/dec by [slope] [y-units]


2)  Intercept 

   b0 = y-val when x=0
      = our prediction for [y-variable] when [x-variale] is zero.
...
If a flower's petal is zero cm long, then we predict that its width will be -0.363 cm?


Uh-oh!  Extrapolation!    "junk in ->  junk out"

```{r}
plot(iris$Petal.Length, iris$Petal.Width)
```

Moral:  only makes predictions for x-vals within the range of observations!

Ex)  Find teh equation for the linear model to predict cty mpg based on displ.
...
```{r}
plot(mpgData$displ, mpgData$cty)
```

Need:

```{r}
r <- cor(mpgData$displ, mpgData$cty)
xbar <- mean(mpgData$displ)
ybar <- mean(mpgData$cty)
sx  <-  sd(mpgData$displ)
sy  <-  sd(mpgData$cty)
```
...
abline(25.992, -2.630)
```
Interpret the coeffs!

x-var:   displ (L)
y-var:  mpg

slope:  For each additional liter bigger the engine is, we predict the cty fuel efficiency to decrease 2.63 mpg.

int:  If a car's engine is 0L big (!!!), we predict the fuel efficiency to be 25.992 mpg.  ( extrapolation).


## lm

Ex:  find equation for lin model to predict sepal width based on sepal length.
...
```{r}
irisModel <- lm(iris$Sepal.Width~iris$Sepal.Length)
```
...
```{r}
hist(irisModel$residuals)
```

We need our residuals to follow a NORMAL distribution!

--> hope the histogram is symmetric, bell-shaped

DON'T WANT:  heavy skew,  multi-modal

Next:  "residual plot":  x-val vs residuals
...
Model:  predict cty mpg based on displ.

```{r}
mpgModel <- lm(mpgData$cty~mpgData$displ)
```
...
residual plot:
         ...
         original x     resid
```{r}
plot( mpgData$displ, mpgModel$residuals  )
```

Want/don't want to see in resid plot:

- DON'T want to see a pattern!  pattern suggests errors are predictable!  suggests linear model isn't the best.

- we DO WANT residuals to be homoskedastic: even spread/magnitude across the range of x-values.  If heteroskedastic:  model is failing systematically in some regions (bad!)
...
In lin reg:
   ...
   r^2 is the percentage of variation in the y data that's becaus of/due to the linear relationship between x and y.
   
Ex:  x=displ, y = cty 

```{r}
cor(mpgData$displ, mpgData$cty)^2
```

We know displ isn't the only factor determining cty mpg.  BUT, of all the variation we see in cty mpg, about 64.8% is because of the lin rel btwn displ and cty mpg.

Ex)  x = cty, y = hwy

```{r}
summary(lm(mpgData$hwy~mpgData$cty))
```

About 91.4% of the variation in hwy mpg is due to the effect/relationship with cty mpg.

## Inference for linear regression

Care about:  correlation coefficient and slope

rho is the "population" correlation.

Goal:  say something smart about rho based on r!

Bad news:  distribution for r is nasty.
...
Ex)  iris.  Is there evidence of a linear relationship btwn sepal length and sepaal width?
...
since p-val > .05, we fail to reject H0.  There's not strong evidence of a linear relationship.

WARNING:  the test for rho can only show that there's SOME lin rel.

DOESN'T SHOW "strong relationship"!!!
...
Suppose we wish to estimate the proportion of DU students who exercise at the gym at least once a week.  We want 95% confidence, and error no greater than 4%.  We don't have any prior sample data to draw on.

How many students must we include in our sample?
...
Meet with another person/group.  Discuss:

1)  What data?
2)  Preliminary observation?  Graphs?  Tests?  Lin reg?
3)  Goals/questions/investigations for project?
