...
Now that we've got the subsets, let's find the proportion per party in each one.  This will be the "true proportion" that we'll used to judge the effectiveness of our confidence intervals by later. A proportion is simply a ratio: the number in our category ($x$) of interest divided by the total ($n$).  In math:
$$\hat{p}=\frac{x}{n}$$
