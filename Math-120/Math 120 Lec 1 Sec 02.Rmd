...
1)  Each row is: an observation/case.  The individual we're collecting data about.
2)  Each column is: a variable.  I.e., a specific quality of the individuals/observations/cases.
3)  Each individual entry is: a value (or cell).
...
- Discrete.  Must be countable.  Must be whole numbers.
  - Examples: #of studetns in this class, # siblings.
- Continuous.  Cts.  Anything that can be measured.  Could be any real number!
  - Examples: height, weight, distance, length of time/duration, volume, mass, etc etc.  All physical measurements are continuous.
...
- Ordinal.  Has an intrinsic order.  
  - Examples: Year in school.  Age in years.  
- Nominal.  
  - Examples: Hair color.  
...
Tricky example:  year is ordinal.  All date words are!

Tricky example:  just because numbers get rounded, that doesn't mean not continuous!

Trick example:  money.  Could argue either way.  Let's agree in Math 120 that money is continuous!
